-- File Name: OE_Shipments_by_Mo_Vsat_Proj_Class_Cat_u2.sql
--
...
@utlspon OE_Shipments_by_Mo_Vsat_Proj_Class_Cat_u2
...
and key.key_name = 'PK_OE_SHIP_PCCID' ;
...
values
  ('PA_Class_Categories_Vsat',
   'FK_OE_SHIP_PCCID',
   'PA_Class_Categories_Vsat view Foreign Key for column label based join relationships.',
   '',
   'FK',
   'COL',
   'Y',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'OE_Shipments_By_Mo_Vsat' and key_name = 'PK_OE_SHIP_PCCID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_OE_SHIP_PCCID' ;
