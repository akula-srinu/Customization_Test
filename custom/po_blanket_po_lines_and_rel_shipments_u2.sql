-- File Name: po_blanket_po_lines_and_rel_shipments_u2.sql
--
...
@utlspon po_blanket_po_lines_and_rel_shipments_u2
...
and key.key_name = 'PK_PO_LINE_REL_SHIPMENTS_ID' ;
...
values
  ('PO_Release_Shipments',
   'FK_PO_LINE_REL_SHIPMENTS_ID',
   'PO_Release_Shipments view Foreign Key for column label based join relationships - Viasat Custom Join',
   '',
   'FK',
   'COL',
   'N',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_Blanket_PO_Lines' and key_name = 'PK_PO_LINE_REL_SHIPMENTS_ID'),
   '*',
   '',
   'Y',
   '',
...
where col.view_label = key.view_label
and col.column_label = 'Line_ID'
and key.view_label = 'PO_Release_Shipments'
and key.key_name = 'FK_PO_LINE_REL_SHIPMENTS_ID';
