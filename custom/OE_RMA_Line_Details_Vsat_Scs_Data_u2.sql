-- File Name: OE_RMA_Line_Details_Vsat_Scs_Data_u2.sql
--
...
@utlspon OE_RMA_Line_Details_Vsat_Scs_Data_u2
...
values
  ('OKS_Scs_Data_Vsat',
   'PK_ORD_LINE_PID',
   'OKS_Scs_Data_Vsat view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.key_name = 'PK_ORD_LINE_PID' ;
...
values
  ('OE_RMA_Line_Details_Vsat',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'OKS_Scs_Data_Vsat' and key_name = 'PK_ORD_LINE_PID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_ORD_LINE_PID' ;
