...
-- File Name: OE_Lines_Details_Proj_u2.sql
--
...
@utlspon OE_Line_Details_Proj_u2
...
and key.key_name = 'PK_ORD_LINE_DET_PID' ;
...
values
  ('PA_Projects',
   'FK_ORD_LINE_DET_PID',
   'PA_Projects view Foreign Key for column label based join relationships.',
   '',
   'FK',
   'COL',
   'Y',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'OE_Line_Details' and key_name = 'PK_ORD_LINE_DET_PID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_ORD_LINE_DET_PID' ;
