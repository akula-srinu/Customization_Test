-- File Name: po_receipts_blanket_pos_u2.sql
--
...
@utlspon po_receipts_blanket_pos_u2
...
values
  ('PO_Blanket_POs',
   'PK_RCPT_BLNK_POS_ID',
   'PO_Blanket_POs view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.key_name = 'PK_RCPT_BLNK_POS_ID' ;
...
values
  ('PO_Receipts',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_Blanket_POs' and key_name = 'PK_RCPT_BLNK_POS_ID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_RCPT_BLNK_POS_ID' ;
