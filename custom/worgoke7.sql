...
--   10-aug-97 W Bonneau  default min_org to -98
...
--
column  min_org_id new_value min_org noprint
column  min_sob_id new_value min_sob noprint
select -98 min_org_id, 
       -98 min_sob_id
  from dual
;

select nvl(min(org_id),-98)          min_org_id,
       nvl(min(set_of_books_id),-98) min_sob_id
  from &PA_USERID..pa_implementations_all
;

--
--  Upon entry n_application_owners is already populated with one record
--  by apps.sql.  Update it to the min(org_id) of all orgs
--  found in this pa_userid.  After this we will create a new record for
--  all additional orgs in this pa_userid.
--
update  n_application_owners n
   set (set_of_books_id,org_id) = 
          (select set_of_books_id,org_id
             from &PA_USERID..pa_implementations_all
            where org_id = &MIN_ORG
               or (     &MIN_ORG         = -98
                    and &MIN_SOB        != -98
                    and set_of_books_id  = &MIN_SOB )
          )
...
select  n.application_label,
        ...
        null
   from &PA_USERID..pa_implementations_all o,
        n_application_owners               n
  where not (          o.org_id           = &MIN_ORG
              or (     &MIN_ORG           = -98
                   and &MIN_SOB          != -98
                   and o.set_of_books_id  = &MIN_SOB ) 
            )
