...
-- File Name: OE_Order_Lines_Task_u2.sql
--
-- Date Created: 30-12-2014
--
-- Purpose: Addition of Columns and joins to the existing view OE_Lines.
--
-- Requested By: Viasat.
--
-- Versions:
...
values
  ('PA_Tasks',
   'PK_ORDER_LINE_TID',
   'PA_Tasks view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.view_label = 'PA_Tasks'
and key.key_name = 'PK_ORDER_LINE_TID' ;
...
values
  ('OE_Lines',
   'FK_ORDER_LINE_TID',
   'OE_Lines view Foreign Key for column label based join relationships.',
   '',
   'FK',
   'COL',
   'Y',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PA_Tasks' and key_name = 'PK_ORDER_LINE_TID'),
   '*',
   '',
   'Y',
   '',
...
from n_view_column_templates col, n_join_key_templates key
where col.view_label = key.view_label
and col.column_label = 'Order_Task_Number'
and key.view_label = 'OE_Lines'
and key.key_name = 'FK_ORDER_LINE_TID' ;
