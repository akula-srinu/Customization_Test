-- File Name: po_releases_and_rel_shipments_u2.sql
--
...
@utlspon po_releases_and_rel_shipments_u2
...
values
  ('PO_Releases',
   'PK_REL_SHIPMENTS_ID',
   'PO_Releases view Primary Key for column_based join relation ships - Viasat Custom Join',
   '',
   'PK',
   'COL',
   '',
...
and key.key_name = 'PK_REL_SHIPMENTS_ID' ;
...
values
  ('PO_Release_Shipments',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_Releases' and key_name = 'PK_REL_SHIPMENTS_ID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_REL_SHIPMENTS_ID';
