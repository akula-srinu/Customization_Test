-- File Name: po_receipts_release_distibutions_u2.sql
--
...
@utlspon po_receipts_release_distibutions_u2
...
values
  ('PO_Release_Distributions',
   'PK_RCPT_REL_DIST_ID',
   'PO_Release_Distributions view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.key_name = 'PK_RCPT_REL_DIST_ID' ;
...
values
  ('PO_Receipts',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_Release_Distributions' and key_name = 'PK_RCPT_REL_DIST_ID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_RCPT_REL_DIST_ID' ;
