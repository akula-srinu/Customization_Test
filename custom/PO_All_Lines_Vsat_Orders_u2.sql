...
-- File Name: PO_All_Lines_Vsat_Orders_u2.sql
--
...
@utlspon PO_All_Lines_Vsat_Orders_u2
...
and key.key_name = 'PK_LINE_ORDER_PID' ;
...
values
  ('PO_All_Orders_Vsat',
   'FK_LINE_ORDER_PID',
   'PO_All_Orders_Vsat view Foreign Key for column label based join relationships.',
   '',
   'FK',
   'COL',
   'N',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_All_Lines_Vsat' and key_name = 'PK_LINE_ORDER_PID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_LINE_ORDER_PID' ;
