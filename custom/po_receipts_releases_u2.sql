-- File Name: po_receipts_releases_u2.sql
--
...
@utlspon po_receipts_releases_u2
...
values
  ('PO_Releases',
   'PK_RCPT_RELEASE_ID',
   'PO_Releases view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.key_name = 'PK_RCPT_RELEASE_ID' ;
...
values
  ('PO_Receipts',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_Releases' and key_name = 'PK_RCPT_RELEASE_ID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_RCPT_RELEASE_ID' ;
