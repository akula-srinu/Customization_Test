...
values
  ('BEN_BEN_Costs01_Vsat',
   'PK_BEN_PER_PID',
   'BEN_BEN_Costs01_Vsat view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.key_name = 'PK_BEN_PER_PID' ;
...
values
  ('BEN_Ben_Costs_Vsat',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'BEN_BEN_Costs01_Vsat' and key_name = 'PK_BEN_PER_PID'),
   '*',
   '',
   'Y',
   '',
