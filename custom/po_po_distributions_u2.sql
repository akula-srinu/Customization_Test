-- File Name: PO_PO_Distributions_u2.sql
--
-- Date Created: 22-12-2014
--
-- Purpose: Addition of Columns and joins to the existing view PO_PO_Distributions.
--
-- Requested By: Viasat.
--
-- Versions:
...
@utlspon PO_PO_Distributions_u2
...
values
  ('PO_Receipts',
   'PK_LINE_LOC_ID',
   'PO_Receipts view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.key_name = 'PK_LINE_LOC_ID' ;
...
values
  ('PO_PO_Distributions',
   'FK_LINE_LOC_ID',
   'PO_PO_Distributions view Foreign Key for column label based join relationships.',
   '',
   'FK',
   'COL',
   'Y',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_Receipts' and key_name = 'PK_LINE_LOC_ID'),
   '*',
   '',
   'Y',
   '',
...
where col.view_label = key.view_label
and col.column_label = 'PO_Line_Location_ID'
and key.view_label = 'PO_PO_Distributions'
and key.key_name = 'FK_LINE_LOC_ID' ;
