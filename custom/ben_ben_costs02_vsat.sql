@utlspon BEN_Ben_Costs_Vsat_New
...
insert into n_join_key_templates
  (VIEW_LABEL,
   ...
   VERSION_ID)
values
  ('BEN_BEN_Costs02_Vsat',
   'PK_BEN_PER_PLAN_PID',
   'BEN_BEN_Costs02_Vsat view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.view_label = 'BEN_BEN_Costs02_Vsat'
and key.key_name = 'PK_BEN_PER_PLAN_PID' ;
...
values
  ('BEN_Ben_Costs_Vsat',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'BEN_BEN_Costs02_Vsat' and key_name = 'PK_BEN_PER_PLAN_PID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_BEN_PER_PLAN_PID' ;
