@utlspon PO_Distributions_RTO_Receipts_u2
...
values
  ('PO_RTO_Receipts_Vsat',
   'PK_DIST_PID_2',
   'PO_RTO_Receipts_Vsat view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and key.view_label = 'PO_RTO_Receipts_Vsat'
and key.key_name = 'PK_DIST_PID_2' ;
...
values
  ('PO_All_Distribution_Vsat',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_RTO_Receipts_Vsat' and key_name = 'PK_DIST_PID_2'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_DIST_RTO_RCPT_PID' ;
