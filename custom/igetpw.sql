...
--   04-Mar-19 G Venkat   Hide/Encrypt apps password. (NV-2238)
--
...
select 'prompt  column C_'||'PW_'||'&APPS_USERID'||' new_value '||'PW_'||'&APPS_USERID'|| ' noprint ' from dual;
select /*+ RULE */
       distinct 
       'prompt select UTL_RAW.cast_to_varchar2(substr('''||'\\\&PW_'||'&APPS_USERID'||''''||
	   ',instr('''||'\\\&PW_'||'&APPS_USERID'||''''||',''/'')'
		from dual;
select 'prompt +1,instr('''||'\\\&PW_'||'&APPS_USERID'||''''||',''###'')-instr('''||'\\\&PW_'||'&APPS_USERID'||''''||',''/'')-1' from dual;
select 'prompt )) C_PW_'||'&APPS_USERID'||' from dual' from dual;
