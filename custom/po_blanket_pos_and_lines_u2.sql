-- File Name: po_blanket_pos_and_lines_u2.sql
--
...
@utlspon po_blanket_pos_and_lines_u2
...
and key.key_name = 'PK_POS_LINES_ID' ;
...
values
  ('PO_Blanket_PO_Lines',
   'FK_POS_LINES_ID',
   'PO_Blanket_PO_Lines view Foreign Key for column label based join relationships - Viasat Custom Join',
   '',
   'FK',
   'COL',
   'N',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'PO_Blanket_POs' and key_name = 'PK_POS_LINES_ID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_POS_LINES_ID';
