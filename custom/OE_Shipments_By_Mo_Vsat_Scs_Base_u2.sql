-- File Name: OE_Shipments_By_Mo_Vsat_Scs_Base_u2.sql
--
...
@utlspon OE_Shipments_By_Mo_Vsat_Scs_Base_u2
...
values
  ('OKS_Scs_Base_Data_Vsat',
   'PK_SHIP_LINE_PCID',
   'OKS_Scs_Base_Data_Vsat view Primary Key for column_based join relation ships.',
   '',
   'PK',
   'COL',
   '',
...
and col.column_label = 'Order_Number'
and key.view_label = 'OKS_Scs_Base_Data_Vsat'
and key.key_name = 'PK_SHIP_LINE_PCID' ;
...
values
  ('OE_Shipments_By_Mo_Vsat',
   ...
   '',
   'N',
   (SELECT T_JOIN_KEY_ID FROM N_JOIN_KEY_TEMPLATES where view_label = 'OKS_Scs_Base_Data_Vsat' and key_name = 'PK_SHIP_LINE_PCID'),
   '*',
   '',
   'Y',
   '',
...
and key.key_name = 'FK_SHIP_LINE_PCID' ;
